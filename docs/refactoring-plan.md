...
管理対象を起点に設定をまとめる方針は維持します。ただし、リンクする設定と、Homebrew や Bun などの環境構築に使う宣言は分けます。前者は `dotfiles/`、後者は `setup/` に置き、配備先との対応は mise の `[dotfiles]` で明示します。この構成では、管理対象ごとの探索範囲を保ちながら、ホームディレクトリへ反映するデータとリポジトリの運用処理を区別できます。

公開コマンドは `bin/`、その内部実装は `libexec/` に置きます。`bin/` のファイル名は利用者が入力するコマンド名なので、実装が shell であっても拡張子を付けません。`libexec/` と `setup/` のシェルスクリプトには `.sh` を付け、公開インターフェースと実装ファイルを名前から判別できるようにします。

ルートには、mise や Lefthook などが標準の探索規則で読み込む制御ファイルを残します。ファイル数を減らすためだけに設定を別ディレクトリへ移し、各ツールへ追加オプションを渡す構成にはしません。
...
- `[dotfiles]` は現在の宣言、ファイルシステム、mise が `symlink-each` 用に記録する状態から判断し、独自の所有権台帳を追加しない
- `symlink-each` は、`~/.local/bin` と `~/.apm` にある管理外の隣接項目を残すために使う
- apm が書き換える lock ファイルは `copy` で配備し、更新後に宣言元へ戻す
...
- [ ] apm の宣言ファイルと更新可能な lock ファイルを別の配備モードに分ける
- [ ] apm が lock ファイルを書き直した後も、`mise bootstrap` を再実行できることを検査する
- [ ] apm の更新結果をリポジトリの lock ファイルへ戻す
- [ ] 拡張子なしにした公開コマンドのラッパー参照を修正し、参照先の存在を検査する
...
- [ ] 移行対象が残っていないと確認できた後、`setup/migrations/` と配備専用の `libexec/path.sh` を削除する
