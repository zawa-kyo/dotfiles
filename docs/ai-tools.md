...
4. `mise -C ~ exec -- apm install -g` を実行する
5. Bun や pre-commit など、既存のセットアップ処理を続ける

`mise -C ~ exec -- apm install -g` は `config/ai/apm/apm.lock.yaml` に従うため、通常のセットアップではスキルのバージョンを勝手に進めない。apm CLI 自体は `config/tools/mise/conf.d/tools.toml` のグローバル mise 設定で管理する。
