...
`~/.local/bin` と `~/.apm` は `symlink-each` を使います。これは管理対象をディレクトリ単位で置き換えず、宣言元の各項目を配備先へ追加する方式です。利用者が同じディレクトリへ置いた管理外ファイルと、apm が生成する `apm_modules/` を残せます。
...
mise は過去の宣言を所有権台帳として保存しません。管理対象を宣言から削除する場合は、先に `mise bootstrap dotfiles unapply` の対象を確認し、必要なリンクを外してから設定を削除します。この順序を守れない変更では、所有元を狭く判定できる一時的な migration を用意します。
