...
func TestLegacyAPMMigrationRejectsForeignLink(t *testing.T) {
	...
	// Run the migration directly to inspect its failure output.
...
func TestLegacyCommandWrapperCleanup(t *testing.T) {
	...
	// Set up the migration fixture and the hook under test.
	mustWriteAPMManifest(t, filepath.Join(fixtureRepo, "dotfiles", "ai", "apm"))
...
// Migration removes only pre-commit hooks generated by superseded tooling.
func TestLegacyGitHookCleanup(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			fixtureRepo := t.TempDir()
			home := t.TempDir()
			hookPath := filepath.Join(fixtureRepo, ".git", "hooks", "pre-commit")

			mustWriteAPMManifest(t, filepath.Join(fixtureRepo, "dotfiles", "ai", "apm"))
			mustWriteFile(t, hookPath, test.content, 0o755)
			env := migrationEnvironment(home)
			env["DIR_GIT_PRE_COMMIT_HOOK"] = hookPath

			// Managed hooks are removed; user-owned hooks stay in place.
			runCommand(t, repo, env, "bash", filepath.Join(repo, miseBootstrapMigration), fixtureRepo)

			if test.shouldKeep {
				assertFileContent(t, hookPath, test.content)
			} else {
				assertPathMissing(t, hookPath)
			}
		})
	}
}
...
func TestBunDataMigration(t *testing.T) {
	for _, sourceKind := range []string{"previous", "legacy"} {
		t.Run(sourceKind, func(t *testing.T) {
			...
			// Create the source layout and the commands required by the migration.
			...
			// Run the migration with all user-controlled paths isolated.
