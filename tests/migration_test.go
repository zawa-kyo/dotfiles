...
func TestLegacyAPMDataMigration(t *testing.T) {
	...
	currentSource := filepath.Join(fixtureRepo, "config", "ai", "apm")
	...
	runCommand(t, repo, migrationEnvironment(home), "bash", filepath.Join(repo, "deploy", "migrations", "001-cleanup-legacy-links.sh"), fixtureRepo)
...
func TestLegacyAPMMigrationRejectsForeignLink(t *testing.T) {
	...
	mustWriteFile(t, filepath.Join(fixtureRepo, "config", "ai", "apm", "apm.yml"), "dependencies: []\n", 0o644)
	mustSymlink(t, foreignSource, target)

	command := exec.Command("bash", filepath.Join(repo, "deploy", "migrations", "001-cleanup-legacy-links.sh"), fixtureRepo)
	command.Dir = repo
...
// TestBunDataMigration verifies generated modules move outside the repository.
func TestBunDataMigration(t *testing.T) {
	repo := repositoryRoot(t)
	source := filepath.Join(t.TempDir(), "bun-source")
	legacySource := filepath.Join(t.TempDir(), "bun-legacy")
	...
	mustWriteFile(t, filepath.Join(legacySource, "node_modules", ".bin", "example"), "#!/usr/bin/env bash\n", 0o755)
	mustWriteFile(t, filepath.Join(fakeBin, "bun"), "#!/usr/bin/env bash\nexit 0\n", 0o755)
	mustSymlink(t, legacySource, globalDir)
	...
	}, "bash", filepath.Join(repo, "tasks", "install-bun.sh"))
	...
	if _, err := os.Lstat(filepath.Join(legacySource, "node_modules")); !os.IsNotExist(err) {
		t.Fatalf("legacy Bun modules remain in the repository: %v", err)
	}
}
