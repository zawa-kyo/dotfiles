...
- `home/.config/nvim/` を変更する場合:
  - まず `home/.config/nvim/lua/policies/` を確認する。
  - キーバインド変更は `home/.config/nvim/lua/policies/keybinds-policy.md` を確認する。
  - タブ / バッファ表示変更は `home/.config/nvim/lua/policies/tab-buffer-policy.md` を確認する。
- `bin/`、`tasks/`、`libexec/`、`mise.toml`、または `home/` 配下のシェル設定を変更する場合:
  - `docs/command-model.md` と `docs/abbreviation-policy.md` を確認する。
  - `fzf` を使う処理は `docs/command-model.md` の共通化方針に従う。
- `packages/homebrew/` を変更する場合:
  - `README.md` の Homebrew 節と `docs/operations.md` を確認する。
- `packages/bun/` を変更する場合:
  - `README.md` の Bun 節と `docs/operations.md` を確認する。
- `home/.codex/`、`home/.claude/`、`packages/apm/`、または `config/ai/codex/rules/` を変更する場合:
...
- AI スキル本体は、原則として外部の apm パッケージリポジトリで管理する。dotfiles には `packages/apm/apm.yml` の依存関係と `packages/apm/apm.lock.yaml` だけを置く。
...
- `packages/homebrew/Brewfile` を変更した場合:
  - `brew bundle check --file=packages/homebrew/Brewfile` を実行する。
- `packages/bun/` を変更した場合:
...
- 新しい Neovim 規約を追加したら `home/.config/nvim/lua/policies/` を更新する。
