...
`mise run install` は dotfiles 管理ファイルのリンク、公開グローバルコマンドの同期、ローカル `mise` ツールのインストール、apm 管理スキルの反映、Bun グローバル環境の準備、リポジトリの pre-commit hook のインストールを行います。
...
- `mise run install` はグローバル mise 設定から apm CLI をインストールし、lock ファイルに固定されたスキルを適用する
