...
- `scripts/utils/`: シェル用の共通 helper
- `homebrew/`: 管理対象の `Brewfile`
- `bun/`: リポジトリ内で管理する Bun グローバルパッケージ
- `procs/`: プロセスビューア設定
- `sheldon/`: シェルプラグインマネージャ設定と abbreviation
- `ai/`: AI ツール設定
...
カスタムテーマは `starship/themes/*.toml` に置きます。`search-theme` または `sT` abbreviation を実行すると、現在のシェルセッションだけテーマを切り替えられます。新しいターミナルを開くと既定テーマに戻ります。
...
- 再利用可能な skills は `apm/apm.yml` の依存関係として管理する
- `mise run install` は `apm install -g` を実行し、lock された skills を適用する
- `mise run upgrade` は apm 管理の skills と `apm/apm.lock.yaml` を更新する
- `mise/conf.d/env.toml` は標準パスを定義する
- custom skills は、英語の commit message など成果物に明示的な言語要件がある場合を除き、ユーザーの依頼言語で応答する
...
`mise` は主要な task entrypoint と discovery interface として使います。

- `mise run install` は標準のローカルセットアップを実行する
- `mise run format` は tracked source files を整形する
- `mise run check-pre-commit` はリポジトリ全体の checks を実行する
- `scripts/global/` の公開コマンドは、生成された `mise` task wrapper 経由でも実行できる
