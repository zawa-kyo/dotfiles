...
bun_dir="${DIR_BUN_SOURCE:-$dotfiles_dir/packages/bun}"
legacy_bun_dir="${DIR_BUN_LEGACY_SOURCE:-$dotfiles_dir/config/tools/bun}"
...
prepare_global_dir() {
  ...
  if [ -L "$modules_dir" ] && [ -e "$global_dir/node_modules" ] &&
    [ "$(realpath "$modules_dir")" = "$(realpath "$global_dir/node_modules")" ]; then
  ...
  if [ "$migrate_modules" = true ]; then
    ...
    mv "$modules_dir" "$global_dir/node_modules"
